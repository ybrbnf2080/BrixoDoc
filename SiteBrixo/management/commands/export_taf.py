...
from SiteBrixo.models import (ArticleOem, Articles, ArticlesToVehicle, File,
                              Marketplaces, Suppliers, VehicleBrands,
                              VehicleModels, Vehicles)

COMPANIES = Suppliers.objects.all()
...
def generate_001(brand_no: str):
    with open(BASE_DIR / 'converted_db' / str(brand_no) / f'001.{brand_no}', 'w', encoding='utf-8') as file:
        data = [
            ...
            '1'.ljust(3),  # TableNo
            '0'.ljust(4),  # Data Release
            '20220209'.ljust(8),  # VersionDate
            '1'.ljust(1),  # Full
            '3870'.ljust(6),  # ManNo
            COMPANIES.get(BrandNo=brand_no).Name.ljust(20),  # BrandName
            '222'.ljust(4),  # RefDataVersion
            ''.ljust(4),  # Reserved
            '2.4'.ljust(3),  # Format
...
def generate_200(brand_no: str):
    objects = Articles.objects.filter(SupplierId__BrandNo=brand_no)
...
def generate_203(brand_no: str):
    objects = ArticleOem.objects.filter()
